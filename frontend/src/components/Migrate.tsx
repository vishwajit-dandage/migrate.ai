"use client";
import React, { useEffect, useState } from "react";
import { Button } from "./ui/button";
import { io } from "socket.io-client";
import { ProviderService } from "../../services/provider";
import {
  useCloudProviderStore,
  useDisabled,
  useMigrationStatus,
  useProviderDataPost,
  useStoreProvider,
} from "@/store/app";
import { cn } from "@/lib/utils";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
const Migrate = () => {
  const { disabled, setDisabled } = useDisabled();
  const { providerData } = useProviderDataPost();
  const { provider } = useCloudProviderStore();
  const { setStatus } = useMigrationStatus();
  const { targetCredData } = useStoreProvider();
  const handleMigrate = async () => {
    setDisabled(true);
    console.log("migrate called");
    console.log(providerData);
    if (targetCredData == null) {
      toast.warn("Please provide target cloud credential", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
        pauseOnHover: true,
        draggable: true,
        progress: undefined,
        theme: "colored",
      });
      setDisabled(false);
      return;
    }
    try {
      console.log("cred gcp", typeof targetCredData);
      console.log("cred gcp obj", targetCredData);
      const test = {
        type: "service_account",
        project_id: "synthetic-cargo-415216",
        private_key_id: "d515f7f3f1fd7af93cde6dd4f85058c4786bb11b",
        private_key:"-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
        client_email:"semicolons-viewer-1@synthetic-cargo-415216.iam.gserviceaccount.com",
        client_id: "107157680358716609706",
        auth_uri: "https://accounts.google.com/o/oauth2/auth",
        token_uri: "https://oauth2.googleapis.com/token",
        auth_provider_x509_cert_url:"https://www.googleapis.com/oauth2/v1/certs",
        client_x509_cert_url:"https://www.googleapis.com/robot/v1/metadata/x509/semicolons-viewer-1%40synthetic-cargo-415216.iam.gserviceaccount.com",
        universe_domain: "googleapis.com",
      }
      const socket = io("http://localhost:5000/", {
        transports: ["websocket"],
        cors: {
          origin: "http://localhost:3000/",
        },
      });
      socket.emit("migrate", providerData, provider, "gcp", test);
      toast.info("Migration has started. Please wait a while", {
        position: "top-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
        pauseOnHover: true,
        draggable: true,
        progress: undefined,
        theme: "light",
      });
      socket.on("result", (data) => {
        console.log("data from backednd");
        console.log(data);
        setStatus(data?.success);
        if (data?.success) {
          toast.success("Migration is completed successfully", {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
            pauseOnHover: true,
            draggable: true,
            progress: undefined,
            theme: "light",
          });
          setDisabled(false);
        } else {
          toast.error("🦄 Something went wrong. Please try again", {
            position: "top-right",
            autoClose: 5000,
            hideProgressBar: false,
            closeOnClick: true,
            pauseOnHover: true,
            draggable: true,
            progress: undefined,
            theme: "light",
          });
          setDisabled(false);
        }
        socket.disconnect();
      });
    } catch (error) {
      console.error("Error during migration:", error);
    }
  };

  return (
    <div className="flex justify-end mt-3">
      <Button
        disabled={disabled}
        className={cn(disabled ? "hover:cursor-wait" : "hover:cursor-pointer")}
        onClick={handleMigrate}
      >
        Migrate
      </Button>
      <ToastContainer />
    </div>
  );
};

export default Migrate;
